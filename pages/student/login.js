...
const Login = () => {
        ...
        .then((userCredential) => {
          ...
          if (user === null) {
            if (userCredential.user.emailVerified) {
              ...
              //   number: userCredential.user.phoneNumber, // Assuming phoneNumber is stored in Firebase
              // };
  ...
  async function loadData() {
    ...
    var additionalModules = Object.keys(fbc.STUDENTMODULE.modules).map(
      (key) => {
        return key;
      }
    );
    ...
    try {
      ...
      utility.store_newvalue(constants.USER_FULLNAME, data[fbc.STUDENT_NAME]);
      ...
      // Store user details in localStorage
      const userDetails = {
        uid: data[fbc.STUDENT_UID],
        name: data[fbc.STUDENT_NAME],
        email: data[fbc.STUDENT_EMAILADDRESS],
        // number: data[fbc.STUDENT_PHONENUMBER], // Assuming phoneNumber is stored in Firestore
      };
      localStorage.setItem('userDetails', JSON.stringify(userDetails));
  ...
  async function updateUserSession() {
    ...
    await setDoc(
      doc(
        db,
        'LoginSessionDetails/student/' + user.uid + '/',
        timestamp.toString()
      ),
      loginData
    );
