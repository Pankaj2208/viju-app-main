...
const Login = () => {
  ...
  async function loadData() {
    ...
    var additionalModules = Object.keys(fbc.PARENTMODULE.modules).map((key) => {
      return key;
    });
    var modules = [
      'PARENTMODULE',
    ...
    try {
      var userRef = doc(db, fbc.PARENT_COLLECTION, user.uid);
      const docSnap = await getDoc(userRef);
      var data = docSnap.data();
      utility.store_newvalue(constants.USER_ID, data[fbc.PARENT_UID]);
      utility.store_newvalue(constants.USER_FULLNAME, data[fbc.PARENT_NAME]);
      utility.store_newvalue(
        constants.USER_LOGINID,
        data[fbc.PARENT_EMAILADDRESS]
      );
      ...
      if (data[fbc.PARENT_UID].length === 0) {
        // window.history.replaceState(null, null, '/student/select-course');
        // window.location = '/student/select-course';
      } else {
        window.history.replaceState(null, null, '/parent/home');
        window.location = '/parent/home';
      }
    } catch (err) {
      utility.hideloading();
  ...
  async function updateUserSession() {
    ...
    await setDoc(
      doc(
        db,
        'LoginSessionDetails/parent/' + user.uid + '/',
        timestamp.toString()
      ),
      loginData
    );
  ...
  function registerParent() {
    window.history.replaceState(null, null, '../parent/register');
