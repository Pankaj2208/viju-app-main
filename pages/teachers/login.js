...
const Login = () => {
  ...
  async function loadData() {
    ...
    var additionalModules = Object.keys(fbc.TEACHERMODULE.modules).map(
      (key) => {
        return key;
      }
    );
    ...
    try {
      var userRef = doc(db, fbc.TEACHER_COLLECTION, user.uid);
      const docSnap = await getDoc(userRef);
      var data = docSnap.data();
      utility.store_newvalue(constants.USER_ID, data[fbc.TEACHER_UID]);
      utility.store_newvalue(constants.USER_FULLNAME, data[fbc.TEACHER_NAME]);
      utility.store_newvalue(
        constants.USER_LOGINID,
        data[fbc.TEACHER_EMAILADDRESS]
      );
  ...
  async function updateUserSession() {
    ...
    await setDoc(
      doc(
        db,
        'LoginSessionDetails/teachers/' + user.uid + '/',
        timestamp.toString()
      ),
      loginData
    );
  ...
  function registerTeacher() {
    window.history.replaceState(null, null, '../teachers/register');
    window.location = '../teachers/register';
  }
