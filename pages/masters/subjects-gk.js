...
import AddorUpdateSubject from '../../components/masters/add-update-subject -Gk';
...
const Subjects = () => {
  ...
  function viewSubject(Gksubject_uid) {
    var subDoc = null;
    allSubjectDocs.map((subjectDoc, index) => {
      if (subjectDoc[fbc.GKSUBJECT_UID] === Gksubject_uid) {
        subDoc = subjectDoc;
      }
    });
    ...
    window.open(
      window.location.origin +
        ...
        subDoc[fbc.GKSUBJECT_COURSE].uid +
        '&subject=' +
        Gksubject_uid,
      '_blank'
    );
  }
  function modifySubject(Gksubject_uid) {
  ...
  async function getAllSubjects() {
    ...
    try {
      ...
      let q = query(SubjectReF, ...params, orderBy(fbc.GKSUBJECT_CODE));
      ...
      allSubjectDocs.map((subjectDoc, index) => {
        ...
        var rowitem = `<tr>
                        ...
                        <td class="text-gray-800 mb-1">${
                          subjectDoc[fbc.GKSUBJECT_COURSE].name
                        }</td>
                        <td class="text-gray-800 mb-1">${
                          subjectDoc[fbc.GKSUBJECT_NAME]
                        }</td>
                        <td class="text-gray-800 mb-1">${
                          subjectDoc[fbc.GKSUBJECT_CODE]
                        }</td>

                        <td>
                          ${
                            subjectDoc[fbc.GKSUBJECT_STATUS] ? active : inactive
                          }
                        </td>

                        <td class="text-end">
                          <button id="modify_${
                            subjectDoc[fbc.GKSUBJECT_UID]
        ...
        $('#modify_' + subjectDoc[fbc.GKSUBJECT_UID]).on('click', function () {
          modifySubject(this.id.replaceAll('modify_', ''));
        });
        $('#view_' + subjectDoc[fbc.GKSUBJECT_UID]).on('click', function () {
