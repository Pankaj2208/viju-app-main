...
import AddorUpdateCourse from '../../components/masters/add-update-courses -Gk';
...
const Courses = () => {
  ...
  function viewCourse(Gkcourse_uid) {
    window.open(
      window.location.origin +
        '/masters/course-details-gk?view=' +
        Gkcourse_uid,
      '_blank'
    );
  }
  function modifyCourse(Gkcourse_uid) {
    setselectCourse((user) => {
      var empDoc = null;
      allCourseDocs.map((courseDoc, index) => {
        if (courseDoc[fbc.GKCOURSE_UID] === Gkcourse_uid) {
          empDoc = courseDoc;
        }
      });
      //console.log({ Gkcourse_uid, empDoc });
      return empDoc;
    });
    // addCourseModal.show();
  }
      ...
      allCourseDocs.map((courseDoc, index) => {
        ...
        var rowitem = `<tr>
                        ...
                        <td class="text-gray-800 mb-1">${
                          courseDoc[fbc.GKCOURSE_NAME]
                        }</td>
                        <td class="text-gray-800 mb-1">${
                          courseDoc[fbc.GKCOURSE_CODE]
                        }</td>
                        <td class="text-gray-800 mb-1">${
                          courseDoc[fbc.GKCOURSE_BOARD].name
                        }</td>

                        <td>
                          ${courseDoc[fbc.GKCOURSE_STATUS] ? active : inactive}
                        </td>

                        <td class="text-end">
                          <button id="modify_${
                            courseDoc[fbc.GKCOURSE_UID]
        ...
        $('#modify_' + courseDoc[fbc.GKCOURSE_UID]).on('click', function () {
          modifyCourse(this.id.replaceAll('modify_', ''));
        });
        $('#view_' + courseDoc[fbc.GKCOURSE_UID]).on('click', function () {
...
export async function getStaticProps() {
  return {
    props: { module: 'GKCOURSEMASTER', onlyAdminAccess: false },
  };
}
