...
import { RequestaddorUpdateGkAssessment } from '@/firebase/masterAPIS';
import {
  arrayUnion,
  collection,
  deleteField,
...
//import { GKQUESTION_ANS } from '@/constants/appconstants'; // Adjust the path as necessary
...
const EditAssessment = () => {
  ...
  useEffect(() => {
    ...
    $('#practiceassessmentswitch').change(function () {
      if (this.checked) {
        $('#practiceassessment_text').text(
          'Practice Assessment, Duration Will Be Ignored'
        );
      } else {
    ...
    setquestionchoices(
      new Choices($('#questionselect')[0], {
        ...
        classNames: {
          containerInner:
            'choices__inner rounded form-control-solid border-0 text-dark fw-bold text-sm',
  ...
  useEffect(() => {
    if (asmtDetails !== null && asmtDetails !== undefined) {
      $('#asmtcode').val(asmtDetails[GKASMT_CODE]);
      $('#asmttitle').val(asmtDetails[GKASMT_TITLE]);
      // $("#generalinstructions").val(asmtDetails[GKASMT_GENERALINSTRUCTIONS])
      // $("#asmtduration").val(asmtDetails[GKASMT_DURATION])
      ...
      setaddedQuestions(asmtDetails[GKASMT_QUESTIONS]);
    }
  }, [asmtDetails]);
  ...
  useEffect(() => {
    if (questionchoices !== null && questionchoices !== undefined) {
      ...
      allQuestionsForLesson.map((question) => {
        array.push({
          value: question[GKQUESTION_UID],
          label: question[GKQUESTION_TEXT] + question[GKQUESTION_ANS],
  ...
  async function getAsmtDetails(asmtuid) {
    const docRef = doc(db, GKASMT_COLLECTION, asmtuid);
  ...
  async function getAllQuestionForLessons(lessonuid) {
    const docRef = doc(db, GKLESSON_COLLECTION, lessonuid);
    const docSnap = await getDoc(docRef);
    ...
    utility.showloading();
    try {
      const questionReF = collection(db, GKQUESTION_COLLECTION);
      const q = query(
        questionReF,
        where(GKQUESTION_LESSONSUID, 'array-contains', lessonuid),
        orderBy(GKQUESTION_CODE)
      );
  ...
  function addQuestion() {
    ...
    var question = {
      uid: questionData[GKQUESTION_UID],
      //type: questionData[QUESTION_TYPE],
      //subtype: questionData[QUESTION_SUBTYPE],
      text: questionData[GKQUESTION_TEXT],
      ans: questionData[GKQUESTION_ANS],
      //onlyassessment: questionData[QUESTION_ONLYASSESSMENT],
      code: questionData[GKQUESTION_CODE],
    ...
    setaddedQuestions((questions) => {
      ...
      updatesQuestion[questionData[GKQUESTION_UID]] = question;
  ...
  useEffect(() => {
    ...
    $('#practiceassessmentswitch').attr(
      'disabled',
      Object.keys(addedQuestions).length > 0
    );
    $('#negativemarkingswitch').attr(
      'disabled',
      Object.keys(addedQuestions).length > 0
    );
  }, [addedQuestions]);
      ...
      utility.info_alert(
        ...
        async () => {
          await setDoc(
            docRef,
            {
              [GKASMT_QUESTIONS + '.' + questionID]: deleteField(),
  ...
  async function saveAssessment() {
    if (checkifDataisCorrect()) {
      ...
      utility.info_alert(
        (asmtUID !== null ? 'Update?' : 'Add') + ' Assessment?',
        'Are you sure you want to continue?',
        'CONTINUE',
        'CANCEL',
        () => {
      ...
      var uploadData = async function () {
        ...
        var data = {
          [GKASMT_UID]: asmtuid,
          [GKASMT_LESSON]: {
            //name: lesson[GKLESSON_NAME],
            //code: lesson[GKLESSON_CODE],
            //uid: lesson[GKLESSON_UID],
          },
          [GKASMT_STATUS]: true,
          // [GKASMT_SUBJECT]: lesson[LESSON_SUBJECT],
          //[GKASMT_COURSE]: lesson[LESSON_COURSE],
          [GKASMT_CODE]: utility.getinputValue('asmtcode'),
          [GKASMT_TITLE]: utility.getinputValue('asmttitle'),
          ...
          [GKASMT_QUESTIONS]: addedQuestions,
        };

        utility.showloading();

        var addorUpdateAssessment = await RequestaddorUpdateGkAssessment(
          data,
          log
        );
        utility.hideloading();
        if (addorUpdateAssessment.status) {
          ...
          var message =
            'Failed To Add Assessment, ' + addorUpdateAssessment.message;
          showsnackbar('error', message);
        }
      };
    }
                              ...
                              <td className="text-dark py-2 px-2 fs-6 fw-bolder align-middle col-1">
                                <a
                                  onClick={(e) => {
                                    console.log(e);
                                    deleteQuestion(
                                      e.target.id.replaceAll('_delete', '')
                                    );
                                  }}
            ...
            <div className="modal-body">
              <div className="row">
                <div className="fv-row mb-7 col-md-12">
                  ...
                  <select
                    id="questionselect"
                    className="text-sm rounded form-control form-control-solid form-control-sm"
                  ></select>
                </div>
