...
const Questions = () => {
  ...
  async function getAllSubjects(Gkcourse_uid) {
    $('#contentdiv').empty();
    console.log(Gkcourse_uid);
    utility.showloading();
    try {
      const ref = collection(db, fbc.GKSUBJECT_COLLECTION);
      var allSubjects = [];
      var params = [];
      ...
      const q = query(
        ref,
        ...params,
        where(fbc.GKSUBJECT_COURSE + '.uid', '==', Gkcourse_uid),
        orderBy(fbc.GKSUBJECT_CODE)
      );
      ...
      allSubjects.map((board) => {
        subjectArray.push({
          value: board[fbc.GKSUBJECT_UID],
          customProperties: board,
          label: board[fbc.GKSUBJECT_NAME],
        });
      });
  ...
  async function getAllLessons(Gksubject_uid) {
    $('#contentdiv').empty();
    console.log(Gksubject_uid);
    utility.showloading();
    try {
      const ref = collection(db, fbc.GKLESSON_COLLECTION);
      var allLessons = [];
      ...
      if (!utility.get_keyvalue(USER_ISADMIN)) {
        params.push(
          where(
            fbc.GKLESSON_ASSESMENTUIDARRAY,
            'array-contains',
            utility.get_keyvalue(USER_ID)
          )
        );
      ...
      const q = query(
        ref,
        ...params,
        where(fbc.GKLESSON_SUBJECT + '.uid', '==', Gksubject_uid),
        orderBy(fbc.GKLESSON_CODE)
      );
      const querySnapshot = await getDocs(q);
      querySnapshot.forEach((doc) => {
      ...
      allLessons.map((board) => {
        lessonArray.push({
          value: board[fbc.GKLESSON_UID],
          customProperties: board,
          label: board[fbc.GKLESSON_NAME],
        });
      });
  ...
  async function getAllCourses(Gkboard_uid) {
    $('#contentdiv').empty();
    utility.showloading();
    try {
      const ref = collection(db, fbc.GKCOURSE_COLLECTION);
      var allCourses = [];
      ...
      if (!utility.get_keyvalue(USER_ISADMIN)) {
        params.push(
          where(
            fbc.GKCOURSE_ACCESSUSERUIDARRAY,
            'array-contains',
            utility.get_keyvalue(USER_ID)
          )
        );
      ...
      const q = query(
        ref,
        ...params,
        where(fbc.GKCOURSE_BOARD + '.uid', '==', Gkboard_uid),
        orderBy(fbc.GKCOURSE_NAME)
      );
      const querySnapshot = await getDocs(q);
      querySnapshot.forEach((doc) => {
      ...
      allCourses.map((board) => {
        courseArray.push({
          value: board[fbc.GKCOURSE_UID],
          customProperties: board,
          label: board[fbc.GKCOURSE_NAME],
        });
      });
  ...
  async function getAllBoards() {
    ...
    utility.showloading();
    try {
      const BoardReF = collection(db, fbc.GKBOARD_COLLECTION);
      var allBoards = [];
      var params = [];
      if (!utility.get_keyvalue(USER_ISADMIN)) {
        params.push(
          where(
            fbc.GKBOARD_ACCESSUSERUIDARRAY,
            'array-contains',
            utility.get_keyvalue(USER_ID)
          )
        );
      }
      let q = query(BoardReF, ...params, orderBy(fbc.GKBOARD_NAME));
      const querySnapshot = await getDocs(q);
      console.log('size L ' + querySnapshot.size);
      querySnapshot.forEach((doc) => {
        allBoards.push({
          details: {
            field: doc.data()[fbc.GKBOARD_FIELD],
            code: doc.data()[fbc.GKBOARD_CODE],
            uid: doc.id,
            name: doc.data()[fbc.GKBOARD_NAME],
          },
          uid: doc.id,
          name: doc.data()[fbc.GKBOARD_NAME],
        });
  ...
  async function getAllQuestions(Gklesson_uid) {
    $('#contentdiv').empty();
    console.log(Gklesson_uid);
    utility.showloading();
    try {
      const QuestionReF = collection(db, fbc.GKQUESTION_COLLECTION);
      var allQuestions = [];
      const q = query(
        QuestionReF,

        where(fbc.GKQUESTION_LESSONSUID, 'array-contains', Gklesson_uid),
        orderBy(fbc.GKQUESTION_CODE)
      );
      const querySnapshot = await getDocs(q);
      ...
      allQuestionDocs.map((questionDoc, index) => {
        ...
        var rowitem = `<tr>
                        ...
                        <td class="text-gray-800 mb-1">${
                          questionDoc[fbc.GKQUESTION_TYPE]
                        }</td>
                        <td class="text-gray-800 mb-1">${
                          questionDoc[fbc.GKQUESTION_CODE]
                        }</td>
                        <td class="text-gray-800 mb-1">${
                          questionDoc[fbc.GKQUESTION_TEXT]
                        }</td>

                        <td>
                          ${
                            questionDoc[fbc.GKQUESTION_STATUS]
                              ? active
                              : inactive
                          }
                        </td>
                        ...
                        <td class="text-end">
                          <button
                          data-type="${questionDoc[fbc.GKQUESTION_TYPE]}"
                          id="modify_${
                            questionDoc[fbc.GKQUESTION_UID]
        ...
        $('#modify_' + questionDoc[fbc.GKQUESTION_UID]).on(
          'click',
          function () {
            modifyQuestion(
  ...
  function uploadexcel() {
    window.open(
      window.location.origin + '/masters/questions-upload-Gk',
      '_blank'
    );
  }
  return (
