...
const Questions = () => {
  ...
  async function getAllLessons() {
    ...
    try {
      ...
      const q = query(lessonReF, orderBy(LESSON_NAME));
      const querySnapshot = await getDocs(q);
      querySnapshot.forEach((doc) => {
        ...
        allLessons[doc.data()[LESSON_CODE].toString().toUpperCase()] = {
          ...
          name:
            doc.data()[LESSON_NAME] +
            ' | ' +
            doc.data()[LESSON_SUBJECT].name +
            ' | ' +
            doc.data()[LESSON_COURSE].name,
        };
      });
    } catch (error) {
      errorCallback(error);
    ...
    Object.keys(data).map((key) => {
      try {
        data[key].map((value) => {
          ...
          if (
            value['ASSESSMENT CODE'].toString().trim().length > 0 &&
            (value['MARKS'].toString().trim().length == 0 ||
              Number(value['MARKS'].toString().trim()) <= 0)
          ) {
          ...
          var question = {
            ['assessment']: {
              ...
              negativemarks:
                Number(value['NEGATIVE MARKS'].toString().trim()) || 0,
            },
  ...
  async function addOQA_Questions(question, quesindex) {
    ...
    var uploadQuestion = async () => {
      ...
      await setDoc(
        doc(db, QUESTION_COLLECTION, questionuid),
        { questionData },
        { merge: true }
      );
  ...
  async function addMCQ_FTB_MTF_Questions(question, quesindex) {
    ...
    var uploadQuestion = async () => {
      ...
      await setDoc(doc(db, QUESTION_COLLECTION, questionuid), questionData, {
        merge: true,
      });
  ...
  async function addQuestionToAsmt(asmt, question) {
    ...
    await setDoc(
      doc(db, ASMT_COLLECTION, asmtUID),
      {
    ...
    console.log(
      'ASMT ADDED QUESTION ' + asmtUID + ' : ' + question[QUESTION_UID]
    );
  }
