...
import AddorUpdateBoard from '../../components/masters/add-update-boards -Gk';
...
const Boards = () => {
  ...
  function modifyBoard(board_uid) {
    setselectBoard((user) => {
      ...
      allBoardDocs.map((boardDoc, index) => {
        if (boardDoc[fbc.GKBOARD_UID] === board_uid) {
          empDoc = boardDoc;
        }
      });
  ...
  async function getAllBoards() {
    ...
    try {
      ...
      var allBoards = [];
      const q = query(BoardReF, orderBy(fbc.GKBOARD_NAME));
      ...
      allBoardDocs.map((boardDoc, index) => {
        ...
        var rowitem = `<tr>
                        ...
                        <td class="text-gray-800 mb-1">${
                          boardDoc[fbc.GKBOARD_NAME]
                        }</td>
                        <td class="text-gray-800 mb-1">${
                          boardDoc[fbc.GKBOARD_CODE]
                        }</td>

                        <td>
                          ${boardDoc[fbc.GKBOARD_STATUS] ? active : inactive}
                        </td>

                        <td class="text-end">
                          <button id="modify_${
                            boardDoc[fbc.GKBOARD_UID]
        ...
        $('#modify_' + boardDoc[fbc.GKBOARD_UID]).on('click', function () {
          modifyBoard(this.id.replaceAll('modify_', ''));
        });
        $('#view_' + boardDoc[fbc.GKBOARD_UID]).on('click', function () {
...
export async function getStaticProps() {
  return {
    props: { module: 'GKBOARDMASTER', onlyAdminAccess: false },
  };
}
