...
import {
  ...
  GKLESSON_SUBJECT,
  GKQUESTION_CODE,
  GKQUESTION_COLLECTION,
  GKQUESTION_COMPLEXITY,
  GKQUESTION_COMPREHENSION,
  GKQUESTION_IMAGEURLS,
  GKQUESTION_LESSONSUID,
  GKQUESTION_LOGS,
  GKQUESTION_ONLYASSESSMENT,
  GKQUESTION_OPTIONS,
  GKQUESTION_STATUS,
  GKQUESTION_SUBTYPE,
  GKQUESTION_TEXT,
...
const Questions = () => {
  ...
  async function getAllAssessments() {
    ...
    utility.showloading();
    try {
      const asmtReF = collection(db, GKASMT_COLLECTION);

      const q = query(asmtReF);
      const querySnapshot = await getDocs(q);
      querySnapshot.forEach((doc) => {
        asmtCodes[doc.data()[GKASMT_CODE].toString().toUpperCase()] =
          doc.data();
        //[(doc.data()[ASMT_CODE] = A1 || A2 || A3)];
      });
    } catch (error) {
      errorCallback(error);
    }
  ...
  async function getAllLessons() {
    ...
    utility.showloading();
    try {
      const lessonReF = collection(db, GKLESSON_COLLECTION);

      const q = query(lessonReF, orderBy(GKLESSON_NAME));
      const querySnapshot = await getDocs(q);
      querySnapshot.forEach((doc) => {
        console.log(doc.data()[GKLESSON_CODE].toString().toUpperCase());
        allLessons[doc.data()[GKLESSON_CODE].toString().toUpperCase()] = {
          lessoncode: doc.data()[GKLESSON_CODE],
          lessonuid: doc.id,
          subjectuid: doc.data()[GKLESSON_SUBJECT].uid,
          courseuid: doc.data()[GKLESSON_SUBJECT].uid,
          name:
            doc.data()[GKLESSON_NAME] +
            ' | ' +
            doc.data()[GKLESSON_SUBJECT].name +
            ' | ' +
            doc.data()[GKLESSON_COURSE].name,
        };
      });
    } catch (error) {
      errorCallback(error);
  ...
  function browseMultipleImages() {
    ...
    input.onchange = (_) => {
      ...
      allFiles.map((file) => {
        if (
          !excelImageFileNames.includes(file.name.split('.')[0].toUpperCase())
        ) {
      ...
      showsnackbar(
        'success',
        excelImageFileNames.length +
          ' Images Selected, Select Question Excel Now.'
      );
    ...
    input.onchange = (_) => {
      ...
      setuploadDetails((saveddetails) => {
        ...
        details['file'] = {
          ...
          lastModifiedDate: moment(files[0].lastModifiedDate).format(
  ...
  function formatExcelData(data) {
    ...
    Object.keys(data).map((key) => {
      try {
        data[key].map((value) => {
          if (
            value['QUESTION TEXT'].toString().trim().length === 0 &&
            value['QUESTION IMAGES'].toString().trim().length === 0
          ) {
            showsnackbar(
              'error',
              'QUESTION TEXT, QUESTION IMAGES not Found, Invalid Excel Format, Please check your Excel Sheet.'
            );
            return;
          }
          if (value['QUESTION CODE'] === undefined) {
            showsnackbar(
              'error',
              'QUESTION CODE not Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                value['QUESTION TEXT']
            );
            return;
          }
          if (
            !Object.keys(QUESTIONTYPE).includes(
              value['QUESTION TYPE'].toString().trim().toUpperCase()
            )
          ) {
            showsnackbar(
              'error',
              'QUESTION TYPE not Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                value['QUESTION TEXT']
            );
            return;
          }
          if (
            !['HIGH', 'MEDIUM', 'LOW'].includes(
              value['QUESTION COMPLEXITY'].toString().trim().toUpperCase()
            )
          ) {
            showsnackbar(
              'error',
              'QUESTION COMPLEXITY not Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                value['QUESTION TEXT']
            );
            return;
          }
          if (
            value['QUESTION SUBTYPE'].toString().trim().length > 0 &&
            !['SINGLE', 'MULTIPLE'].includes(
              value['QUESTION SUBTYPE'].toString().trim().toUpperCase()
            )
          ) {
            showsnackbar(
              'error',
              'QUESTION SUBTYPE not Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                value['QUESTION TEXT']
            );
            return;
          }
          ...
          if (value['LESSON CODES'].toString().trim().split(',').length === 0) {
            showsnackbar(
              'error',
              'LESSON CODES not Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                value['QUESTION TEXT']
            );
            return;
          }
          ...
          if (
            value['ASSESSMENT CODE'].toString().trim().length > 0 &&
            allASMTCodes[
              value['ASSESSMENT CODE'].toString().trim().toUpperCase()
            ] === undefined
          ) {
            showsnackbar(
              'error',
              'ASSESSMENT CODE Is Not Valid, No Such Assessment Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                value['QUESTION TEXT']
            );
            return;
          }
          if (
            value['ASSESSMENT CODE'].toString().trim().length > 0 &&
            (value['QUESTION CODE'].toString().trim().length == 0 ||
              Number(value['QUESTION CODE'].toString().trim()) <= 0)
          ) {
            console.log('367' + value['ASSESSMENT CODE'].toString());
            showsnackbar(
              'error',
              'MARKS not Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                value['QUESTION TEXT']
            );
            return;
          }
          if (
            ...
            showsnackbar(
              'error',
              'NEGATIVE MARKS Invalid, Invalid Excel Format, Please check your Excel Sheet, For ' +
                value['QUESTION TEXT']
            );
            return;
          }
          ...
          if (value['QUESTION IMAGES'].toString().trim().length > 0) {
            for (let imagefilename of value['QUESTION IMAGES']
              .toString()
              .replaceAll(' ', '')
              .split(',')) {
              if (excelImageFileNames.includes(imagefilename)) {
                questionImages.push(
                  excelImageFiles[excelImageFileNames.indexOf(imagefilename)]
                );
              } else {
                ...
                showsnackbar(
                  'error',
                  imagefilename +
                    ', QUESTION IMAGES not Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                    value['QUESTION TEXT']
                );
                return;
              }
          ...
          var option1 = null;
          if (
            value['OPT1 TEXT'].toString().trim().length > 0 ||
            value['OPT1 IMAGE'].toString().trim().length > 0
          ) {
            option1 = {
              ...
              image: '',
              iscorrect:
                value['OPT1 ISCORRECT'].toString().trim().toUpperCase() ===
                ('Y' || 'YES'),
            };
            ...
            if (value['OPT1 IMAGE'].toString().trim().length > 0) {
              ...
              if (excelImageFileNames.includes(imagefilename)) {
                ...
                showsnackbar(
                  'error',
                  'OPT1 IMAGE not Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                    value['QUESTION TEXT']
                );
                return;
              }
          ...
          var option2 = null;
          if (
            value['OPT2 TEXT'].toString().trim().length > 0 ||
            value['OPT2 IMAGE'].toString().trim().length > 0
          ) {
            option2 = {
              uid: utility.randomstring() + '_' + utility.getTimestamp(),
              ...
              image: '',
              iscorrect:
                value['OPT2 ISCORRECT'].toString().trim().toUpperCase() ===
                ('Y' || 'YES'),
            };
            ...
            if (value['OPT2 IMAGE'].toString().trim().length > 0) {
              ...
              if (excelImageFileNames.includes(imagefilename2)) {
                option2['image'] =
                  excelImageFiles[excelImageFileNames.indexOf(imagefilename2)];
              } else {
                showsnackbar(
                  'error',
                  'OPT2 IMAGE not Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                    value['QUESTION TEXT']
                );
                return;
              }
          ...
          var option3 = null;
          if (
            value['OPT3 TEXT'].toString().trim().length > 0 ||
            value['OPT3 IMAGE'].toString().trim().length > 0
          ) {
            option3 = {
              uid: utility.randomstring() + '_' + utility.getTimestamp(),
              ...
              image: '',
              iscorrect:
                value['OPT3 ISCORRECT'].toString().trim().toUpperCase() ===
                ('Y' || 'YES'),
            };
            ...
            if (value['OPT3 IMAGE'].toString().trim().length > 0) {
              ...
              if (excelImageFileNames.includes(imagefilename3)) {
                option3['image'] =
                  excelImageFiles[excelImageFileNames.indexOf(imagefilename3)];
              } else {
                showsnackbar(
                  'error',
                  'OPT3 IMAGE not Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                    value['QUESTION TEXT']
                );
                return;
              }
          ...
          var option4 = null;
          if (
            value['OPT4 TEXT'].toString().trim().length > 0 ||
            value['OPT4 IMAGE'].toString().trim().length > 0
          ) {
            option4 = {
              uid: utility.randomstring() + '_' + utility.getTimestamp(),
              ...
              image: '',
              iscorrect:
                value['OPT4 ISCORRECT'].toString().trim().toUpperCase() ===
                ('Y' || 'YES'),
            };
            ...
            if (value['OPT4 IMAGE'].toString().trim().length > 0) {
              ...
              if (excelImageFileNames.includes(imagefilename4)) {
                option4['image'] =
                  excelImageFiles[excelImageFileNames.indexOf(imagefilename4)];
              } else {
                showsnackbar(
                  'error',
                  'OPT4 IMAGE not Found, Invalid Excel Format, Please check your Excel Sheet, For ' +
                    value['QUESTION TEXT']
                );
                return;
              }
          ...
          if (options.length === 0) {
            showsnackbar(
              'error',
              'Options Not Added, Invalid Excel Format, Please check your Excel Sheet, For ' +
                value['QUESTION TEXT']
            );
            return;
          }

          var lessonCodes = value['LESSON CODES']
            .toString()
            .trim()
            .toUpperCase()
            .replaceAll(' ', '')
          ...
          for (let code of lessonCodes) {
            if (allLessons[code.toString().toUpperCase()] === undefined) {
              showsnackbar(
                'error',
                'Lesson Code Invalid, For ' + value['QUESTION TEXT']
              );
              return;
            } else {
              lessonuids.push(
                allLessons[code.toString().toUpperCase()].lessonuid
              );
            }
          }
          ...
          var question = {
            ['assessment']: {
              code:
                value['ASSESSMENT CODE'].toString().trim().toUpperCase() || '',
              marks: Number(value['QUESTION CODE'].toString().trim()) || 0,
              negativemarks:
                Number(value['QUESTION CODE'].toString().trim()) || 0,
            },
            ...
            ['subtype']: value['QUESTION SUBTYPE']
              .toString()
              .trim()
              .toUpperCase(),
            ['onlyassessment']:
              value['ONLY ASSESSMENT'].toString().trim().toUpperCase() ===
              ('Y' || 'YES'),
            ['lessonuids']: lessonuids,
            ['lessoncodes']: lessonCodes,
            ['text']: value['QUESTION TEXT'].toString().trim(),
            ['comprehension']: value['QUESTION COMPREHENSION']
              .toString()
              .trim(),
        ...
        showsnackbar(
          'error',
          'Invalid Excel Format, Please check your Excel Sheet.'
        );
  ...
  function loadTable() {
    ...
    excelData.map((question, index) => {
      ...
      var row = `<tr id="row_${index}" >
                        ...
                        <td class="text-sm f-w-500">${
                          question.assessment?.code || ''
                        }</td>
                        <td class="text-sm f-w-500">${
                          question.assessment?.marks || ''
                        }</td>
                        <td class="text-sm f-w-500">${
                          question.assessment?.negativemarks || ''
                        }</td>
  ...
  async function addOQA_Questions(question, quesindex) {
    ...
    var uploadQuestion = async () => {
      ...
      var questionData = {
        [GKQUESTION_UID]: questionuid,
        [GKQUESTION_CODE]: question.code,
        [GKQUESTION_LESSONSUID]: question.lessonuids,
        [GKQUESTION_TEXT]: question.text,
        [GKQUESTION_TYPE]: question.type,
        [GKQUESTION_ONLYASSESSMENT]: question.onlyassessment,
        [GKQUESTION_COMPLEXITY]: question.complexity,
        [GKQUESTION_SUBTYPE]: question.subtype,
        [GKQUESTION_COMPREHENSION]: question.comprehension,
        [GKQUESTION_STATUS]: true,
        [GKQUESTION_IMAGEURLS]: questionImageURLS,
        [GKQUESTION_OPTIONS]: [],
        [GKQUESTION_LOGS]: arrayUnion(log),
      };
      await setDoc(
        doc(db, GKQUESTION_COLLECTION, questionuid),
        { questionData },
        { merge: true }
      );
    ...
    utility.updateloadingstatus(
      'Uploading Question ' + quesindex + '/' + excelData.length
    );
    if (question.images.length > 0) {
      ...
      for (let imageFile of question.images) {
        ...
        var imageBS64 = imageFile.base64
          .replace('data:image/png;base64,', '')
          .replace('data:image/jpeg;base64,', '')
          .replace('data:image/jpg;base64,', '');
        uploadString(uploadRef, imageBS64, 'base64', {
  ...
  async function addMCQ_FTB_MTF_Questions(question, quesindex) {
    ...
    var uploadOptions = async function () {
      async function upload(counter) {
        ...
        if (option.image.toString().length > 0) {
          ...
          var imageBS64 = option.image.base64
            .replaceAll('data:image/png;base64,', '')
            .replaceAll('data:image/jpeg;base64,', '')
            .replaceAll('data:image/jpg;base64,', '');
          await uploadString(uploadRef, imageBS64, 'base64', {
            contentType: 'image/png',
          })
            .then(async (snapshot) => {
    ...
    var uploadQuestion = async () => {
      ...
      var questionData = {
        ...
        [GKQUESTION_STATUS]: true,
        [GKQUESTION_IMAGEURLS]: questionImageURLS,
        [GKQUESTION_OPTIONS]: finalOptions,
        [GKQUESTION_LOGS]: arrayUnion(log),
      };
      await setDoc(doc(db, GKQUESTION_COLLECTION, questionuid), questionData, {
        merge: true,
      });
  ...
  async function addQuestionToAsmt(asmt, question) {
    var questiontoasmt = {
      uid: question[GKQUESTION_UID],
      type: question[GKQUESTION_TYPE],
      subtype: question[GKQUESTION_SUBTYPE],
      text: question[GKQUESTION_TEXT],
      complexity: question[GKQUESTION_COMPLEXITY],
      onlyassessment: question[GKQUESTION_ONLYASSESSMENT],
      code: question[GKQUESTION_CODE],
      marks: asmt.marks,
      negativemarks: asmt.negativemarks,
    };

    var log = {
      message: 'Question Added ' + question[GKQUESTION_CODE],
      name: utility.get_keyvalue(USER_FULLNAME),
      uid: utility.get_keyvalue(USER_ID),
      date: utility.getDateandTime(),
      timestamp: utility.getTimestamp(),
    };
    var asmtUID = allASMTCodes[asmt.code][GKASMT_UID];

    await setDoc(
      doc(db, GKASMT_COLLECTION, asmtUID),
      {
        [GKASMT_TOTAL]: increment(asmt.marks),
        [GKASMT_LOGS]: arrayUnion(log),
        [GKASMT_QUESTIONS]: {
          [question[GKQUESTION_UID]]: questiontoasmt,
        },
      },
      { merge: true }
    );
    console.log(
      'ASMT ADDED QUESTION ' + asmtUID + ' : ' + question[GKQUESTION_UID]
    );
  }

  async function uploadAllQuestions() {
    utility.info_alert(
      `Upload ${excelData.length} Question?`,
            ...
            >
              <div className="card h-100">
                <div className="card-header py-2 px-4">
                  ...
                  <div className="card-toolbar">
                    ...
                    >
                      ...
                      <button
                        className="btn btn-sm fs-5 btn-light-primary  btn-flex btn-center btn-active-light-primary ms-2"
                        id="browsephotosbutton"
                        type="button"
                        onClick={(e) => browseMultipleImages()}
                        // data-bs-target="#modal_adduser"
                      >
                        Upload Images
                      </button>
                      <button
                        ...
                        id="uploadquestionbtn"
                        type="button"
                        onClick={(e) => uploadAllQuestions()}
                        // data-bs-target="#modal_adduser"
                      >
                        Upload Questions
                      </button>
                ...
                <div className="card-body p-4 h-100" id="contentdiv">
                  <div
                    style={{ overflow: 'auto' }}
                    className="d-flex flex-row mw-100 h-100"
                  >
                    <table
