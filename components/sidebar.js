...
const Sidebar = () => {
  ...
  useEffect(() => {
    ...
    if (process.env.NEXT_PUBLIC_USERTYPE === 'EMP') {
      ...
      addModuleSidebar('Grades & Subjects', fbc.COURSESMODULE, 'ri-book-fill');
    } else if (process.env.NEXT_PUBLIC_USERTYPE === 'TCH') {
      addModuleSidebar('Home', fbc.TEACHERMODULE, 'ri-book-fill');
    } else if (process.env.NEXT_PUBLIC_USERTYPE === 'STD') {
      addModuleSidebar('Master', fbc.STUDENTMODULE, 'ri-home-3-fill');
    }
