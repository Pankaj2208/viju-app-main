...
const GiveAssessment = ({ stdasmtDetails, setSTDAsmtDetails }) => {
  ...
  useEffect(() => {
    if (stdasmtDetails !== null && stdasmtDetails !== undefined) {
      ...
      var probableEndtimestamp =
        starttimestamp + Number(stdasmtDetails[fbc.STDASMT_DURATION]) * 60;
      ...
      if (timeremaining > 0) {
        Object.keys(
          stdasmtDetails[fbc.STDASMT_ATTEMPTS][activeAttempt].questions
        ).map((key, index) => {
          addQuestions(
            stdasmtDetails[fbc.STDASMT_ATTEMPTS][activeAttempt].questions[key],
            index + 1
          );
        });
      }
  ...
  function addQuestions(question, index) {
    var questionBtn = ` <button
                          ...
                          aria-selected="true"
                          class="nav-link p-3 btn btn-outline btn-outline-dashed btn-outline-primary btn-active-light-primary">${utility.padwithzero(
                            index
                          )}</button>
`;
    ...
    question.options.map((option, index) => {
      ...
      <label id="option-${option.uid}-${
        question.uid
      }" class="d-flex flex-row align-items-center mb-5 cursor-pointer border rounded py-3 px-6">
                              <span class="form-check form-check-custom form-check-solid">
                                <input id="${option.uid}-${
        question.uid
      }" class="form-check-input" type="radio" name="${question.uid}" value="${
        option.uid
      }" />
                              </span>
    ...
    var questionItem = `
                        <div class="tab-pane fade w-100 h-100 "
                          ...
                          <div class="d-flex flex-column ">
                            ...
                            class="fs-4 fw-bolder text-start text-wrap">${
                              question.text
                            }</span>
                            <span id="comprehensiontext${question.uid}" 
                            class="fs-6 my-2 text-start">${
                            ...
                            ${
                              question.imageurls.length > 0
                                ? ` <div id="questionimagesdiv${question.uid}" class='d-flex flex-wrap gap-2 my-3'>
                            ${questionImages}
                            </div>`
                                : ``
                            }
...
`;
    ...
    $(`input[type=radio][name=${question.uid}]`).change(function () {
      ...
      $(`#pills-question-${questionuid}-btn`).removeClass(
        'btn-outline btn-outline-dashed btn-outline-primary btn-active-light-primary'
      );
      $(`#pills-question-${questionuid}-btn`).addClass(
  ...
  async function submitQuestionAnswer(questionuid, selectedAnswer) {
    var activeAttempt = stdasmtDetails[fbc.STDASMT_ACTIVEATTEMPTUID];
    await setDoc(
      doc(db, fbc.STDASMT_COLLECTION, stdasmtDetails[fbc.STDASMT_UID]),
      {
        stdasmt_attempts: {
          [activeAttempt]: {
            questions: {
    ...
    if (stdasmtDetails[fbc.STDASMT_ISPRACTICE]) {
      ...
      if (!iscorrectAnswer) {
        $(`#option-${selectedAnswer}-${questionuid}`).addClass(
          'bg-light-danger'
        );
      }
  ...
  async function submitAssessment(isforceSubmit = false) {
    if (isforceSubmit) {
      ...
      utility.info_alert(
        'Submit ' + stdasmtDetails[fbc.STDASMT_TITLE] + ' Assessment?',
          ...
          <div className="card w-md-75 w-100 mx-auto mt-5">
                ...
                <div className="d-flex flex-row align-items-center w-100  justify-content-between gap-4">
                  <div className="d-flex flex-column ps-4">
                    <div className="d-flex flex-row">
                      {stdasmtDetails[fbc.STDASMT_ISPRACTICE] ? (
                        <span className="badge badge-success px-3 py-4 fs-7 mb-3 badge-square badge-sm">
                          PRACTICE
                        </span>
                      ) : (
                        <></>
                    ...
                    <span className="fs-2 mb-2 fw-bolder">
                      {stdasmtDetails[fbc.STDASMT_TITLE]}
                    </span>
                    <span className="fs-4 text-muted">
                      {stdasmtDetails[fbc.STDASMT_CODE]}
                    </span>
                  </div>
                  ...
                  <div className="d-flex flex-row">
                    <div className="d-flex flex-row px-4 mx-auto gap-2">
                      <div className="d-flex flex-column pe-4">
                        <span
                          id="duration"
                          className="fs-1 mb-1 fw-bolder text-danger text-center"
                        ></span>
                        <span className="fs-5 text-dark fw-semibold  text-center">
                          Time Left
                        </span>
                      </div>
                    </div>
            ...
            <div className="card-body p-4 d-flex flex-md-row flex-column gap-2">
              <div className="d-flex flex-column col-md-9 p-5 border rounded">
                <div
                  className="tab-content w-100 my-2 p-1"
                  id="items-pills-tabContent"
                ></div>
              </div>
              <div className="d-flex flex-column col-md-3 p-5 border rounded">
                <div className="d-flex flex-column my-2">
                  <button
                    ...
                    className="btn btn-block w-100 btn-primary fs-6 hover-elevate-up"
                  >
                    Submit Assessment
                  </button>
                </div>
                <hr className="text-muted" />
                <span className="fs-5 text-muted fw-semibold mb-4  text-start">
                  Questions
                </span>
                <ul
                  className="nav nav-pills d-flex flex-wrap gap-2 justify-content-start"
                  id="questionbtn-pills-tab"
                  role="tablist"
                ></ul>
              </div>
            </div>
