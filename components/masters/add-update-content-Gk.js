    ...
    if (checkifDataisCorrect()) {
      ...
      var uploadFiles = async function (index) {
        ...
        var uploadRef = ref(
          storage,
          'Content/' +
            addNewContentDetails.course.uid +
            '/' +
            addNewContentDetails.subject.uid +
            '/' +
            addNewContentDetails.lesson.uid +
            '/' +
            filename
        );
            ...
            getDownloadURL(snapshot.ref).then(async (downloadURL) => {
              ...
              if (fileURLs.length === formatFiles.length) {
                ...
                utility.success_alert(
                  ...
                  () => {
                    $(
                      `#content_tabbtn_${addNewContentDetails.lesson.uid}`
                    ).click();
      ...
      var uploadData = async function (index, url) {
        ...
        var log = {
          message:
            'Content ' + (selectedContent !== null ? 'Updated' : 'Added'),
        ...
        var contentObj = {
          [fbc.GKCONTENT_UID]: contentUID,
          [fbc.GKCONTENT_HEADING]: utility.getinputValue('heading'),
          [fbc.GKCONTENT_DESCRIPTION]: utility.getinputValue('description'),
          [fbc.GKCONTENT_ACCESSTYPE]: $('#accesstypeswitch').is(':checked')
            ? 'PAID'
            : 'FREE',
          [fbc.GKCONTENT_COURSE]: addNewContentDetails.course,
          [fbc.GKCONTENT_SUBJECT]: addNewContentDetails.subject,
          [fbc.GKCONTENT_LESSON]: addNewContentDetails.lesson,
          [fbc.GKCONTENT_STATUS]: true,
          [fbc.GKCONTENT_LOGS]: arrayUnion(log),
          [fbc.GKCONTENT_FILE]: {
            ...filedetails,
            url,
          },
        ...
        const lessonRef = doc(
          db,
          fbc.GKLESSON_COLLECTION,
          addNewContentDetails.lesson.uid
        );

        // console.log(addNewContentDetails.course);
        // console.log(addNewContentDetails.subject);
        // console.log(addNewContentDetails.lesson);
        ...
        <div className="modal-content">
          <div className="modal-header p-5" id="modal_addcontent_header">
            <h4 className="fw-bold">
              Add / Modify Content For {addNewContentDetails?.lesson.name}
            </h4>
