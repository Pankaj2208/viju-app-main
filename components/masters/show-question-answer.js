...
const QuestionShowAnswers = ({ stdasmtDetails, setQuestionModal }) => {
  ...
  useEffect(() => {
    if (stdasmtDetails !== null && stdasmtDetails !== undefined) {
      ...
      Object.keys(
        stdasmtDetails[fbc.STDASMT_ATTEMPTS][lastAttempt].questions
      ).map((key, index) => {
        addQuestions(
          stdasmtDetails[fbc.STDASMT_ATTEMPTS][lastAttempt].questions[key],
          index + 1
        );
      });
    }
      ...
      <label id="option-${option.uid}-${
        question.uid
                              ...
                              <div class="d-flex flex-column gap-2 me-0 ms-auto align-items-center">
                                  ${
                                    question.answerselected.includes(option.uid)
                                      ? answerGiven
                                      : ''
                                  }
                                  ${
                                    correctAnswerUIDs.includes(option.uid)
                                      ? correctAnswer
                                      : ''
                                  }
                              </div>
