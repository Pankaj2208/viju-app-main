...
import { RequestaddorUpdateGKSubject } from '../../firebase/masterAPIS';
...
const AddorUpdateSubject = ({
  ...
  useEffect(() => {
    ...
    $('#subjectname').val(selectSubject[fbc.GKSUBJECT_NAME]);
    $('#subjectcode').val(selectSubject[fbc.GKSUBJECT_CODE]);
    keywordchoices.setValue(selectSubject[fbc.GKSUBJECT_KEYWORDS]);
    accesschoices.setChoiceByValue(
      selectSubject[fbc.GKSUBJECT_ACCESSUSERUIDARRAY]
    );
    courseschoices.setChoiceByValue(selectSubject[fbc.GKSUBJECT_COURSE].uid);
    setsubjectThumbnailURL(selectSubject[fbc.GKSUBJECT_THUMBNAILURL]);
    $('#subjectstatusswitch')
      .prop('checked', selectSubject[fbc.GKSUBJECT_STATUS])
      .trigger('change');
    $('#subjecthasasmtswitch')
      .prop('checked', selectSubject[fbc.GKSUBJECT_HASASSESSMENTS])
      .trigger('change');
  }, [selectSubject]);
  ...
  useEffect(() => {
    ...
    setcourseschoices(
      new Choices($('#courseselect')[0], {
        ...
        classNames: {
          containerInner:
            'choices__inner bg-white rounded  text-dark fw-bold text-sm',
  ...
  async function getAllCourses() {
    utility.showloading();
    try {
      const CourseReF = collection(db, fbc.GKCOURSE_COLLECTION);
      var allCourses = [];
      var params = [];
      ...
      let q = query(CourseReF, ...params, orderBy(fbc.GKCOURSE_NAME));
      const querySnapshot = await getDocs(q);
      console.log('size L ' + querySnapshot.size);
      querySnapshot.forEach((doc) => {
        allCourses.push({
          details: {
            board: doc.data()[fbc.GKCOURSE_BOARD],
            field: doc.data()[fbc.GKCOURSE_FIELD],
            code: doc.data()[fbc.GKCOURSE_CODE],
            uid: doc.id,
            name: doc.data()[fbc.GKCOURSE_NAME],
          },
          uid: doc.id,
          name: doc.data()[fbc.GKCOURSE_NAME],
        });
  ...
  useEffect(() => {
    ...
    setaccesschoices(
      new Choices($('#accessselect')[0], {
        ...
        classNames: {
          containerInner:
            'choices__inner rounded bg-light text-dark fw-bold text-sm',
          item: 'choices__item pe-2 mb-0 text-sm',
    ...
    setkeywordchoices(
      new Choices($('#keywordsselect')[0], {
        ...
        classNames: {
          containerInner:
            'choices__inner rounded bg-light bg-input-user text-dark fw-bold text-sm',
          item: 'choices__item pe-2 mb-0 text-xs',
          inputCloned:
            'choices__input--cloned bg-light w-auto text-dark fw-bold text-sm mb-0 px-2',
  ...
  function checkifDataisCorrect() {
    ...
    if (imageFile == null && selectSubject == null) {
      ...
      message = 'Please Select Grade Applicable';
  ...
  const addorUpdateSubject = async () => {
    if (checkifDataisCorrect()) {
      ...
      console.log({ selectSubject });
      var subjectUID =
        selectSubject !== null ? selectSubject[fbc.GKSUBJECT_UID] : ulid();

      var subjectObj = {
        [fbc.GKSUBJECT_UID]: subjectUID,
        [fbc.GKSUBJECT_NAME]: utility.getinputValue('subjectname'),
        [fbc.GKSUBJECT_CODE]: utility.getinputValue('subjectcode'),
        [fbc.GKSUBJECT_COURSE]:
          courseschoices.getValue().customProperties.details,
        [fbc.GKSUBJECT_ACCESSUSERUIDARRAY]: accesschoices.getValue(true),
        [fbc.GKSUBJECT_KEYWORDS]: keywordchoices.getValue(true),
        [fbc.GKSUBJECT_STATUS]: $('#subjectstatusswitch').is(':checked'),
        [fbc.GKSUBJECT_HASASSESSMENTS]: $('#subjecthasasmtswitch').is(
          ':checked'
        ),
      };

      if (selectSubject == null) {
        subjectObj[fbc.GKSUBJECT_LESSONSUIDARRAY] = [];
        subjectObj[fbc.GKSUBJECT_LESSONS] = {};
      }
        ...
        uploadBytes(uploadRef, imageFile)
          .then(async (snapshot) => {
            getDownloadURL(snapshot.ref).then(async (downloadURL) => {
              subjectObj[fbc.GKSUBJECT_THUMBNAILURL] = downloadURL;
              uploadSubject();
            });
          })
          .catch((error) => {
      ...
      var uploadSubject = async () => {
        ...
        var addorUpdateSubject = await RequestaddorUpdateGKSubject(
          subjectObj,
          log
        );
        utility.hideloading();
            ...
            <div className="row">
              <div className="fv-row mb-7 col-md-4">
                <label className="fs-6 fw-semibold mb-2">
                  <span className="required">Grade</span>
                  <i
                    className="ri-information-line ms-1 fs-7"
                    data-bs-toggle="tooltip"
                    title="Subject Belongs To This Grade"
                  ></i>
                </label>

                <select
                  id="courseselect"
                  className="text-sm rounded form-control form-control-sm"
                ></select>
              </div>
