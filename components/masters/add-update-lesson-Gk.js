...
import { RequestaddorUpdateGKLesson } from '../../firebase/masterAPIS';
...
const AddorUpdateLesson = ({
  ...
  useEffect(() => {
    ...
    $('#lessonname').val(selectedLesson[fbc.GKLESSON_NAME]);
    $('#lessoncode').val(selectedLesson[fbc.GKLESSON_CODE]);
    keywordchoices.setValue(selectedLesson[fbc.GKLESSON_KEYWORDS]);
    setlessonThumbnailURL(selectedLesson[fbc.GKLESSON_THUMBNAILURL]);

    $('#lessonstatusswitch')
      .prop('checked', selectedLesson[fbc.GKLESSON_STATUS])
      .trigger('change');
  }, [selectedLesson]);
  ...
  const addorUpdateLesson = async () => {
    if (checkifDataisCorrect()) {
      ...
      console.log({ selectedLesson });
      var lessonUID =
        selectedLesson !== null ? selectedLesson[fbc.GKLESSON_UID] : ulid();

      var lessonObj = {
        [fbc.GKLESSON_UID]: lessonUID,
        [fbc.GKLESSON_NAME]: utility.getinputValue('lessonname'),
        [fbc.GKLESSON_CODE]: utility.getinputValue('lessoncode'),
        [fbc.GKLESSON_KEYWORDS]: keywordchoices.getValue(true),
        [fbc.GKLESSON_STATUS]: $('#lessonstatusswitch').is(':checked'),
      };
        ...
        uploadBytes(uploadRef, imageFile)
          .then(async (snapshot) => {
            getDownloadURL(snapshot.ref).then(async (downloadURL) => {
              lessonObj[fbc.GKLESSON_THUMBNAILURL] = downloadURL;
              uploadLesson();
            });
          })
          .catch((error) => {
      ...
      if (selectedLesson == null) {
        lessonObj[fbc.GKLESSON_COURSE] = addNewLessonDetails.course;
        lessonObj[fbc.GKLESSON_SUBJECT] = addNewLessonDetails.subject;
        lessonObj[fbc.GKLESSON_CONTENT] = {};
        lessonObj[fbc.GKLESSON_ASSESMENTUIDARRAY] = {};
      }
      var uploadLesson = async () => {
        ...
        var addorUpdateLesson = await RequestaddorUpdateGKLesson(
          lessonObj,
          log
        );
        utility.hideloading();
