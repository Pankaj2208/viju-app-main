...
import { RequestaddorUpdateGkBoard } from '../../firebase/masterAPIS';
import * as utility from '../../libraries/utility';
const AddorUpdateBoard = ({ setselectBoard, selectBoard, setBoardModal }) => {
  const { enqueueSnackbar, closeSnackbar } = useSnackbar();
  const [boardThumbnailURL, setboardThumbnailURL] = useState(
  ...
  useEffect(() => {
    ...
    $('#boardname').val(selectBoard[fbc.GKBOARD_NAME]);
    $('#boardcode').val(selectBoard[fbc.GKBOARD_CODE]);

    $('#boardfield').val(selectBoard[fbc.GKBOARD_FIELD]);
    setboardThumbnailURL(selectBoard[fbc.GKBOARD_THUMBNAILURL]);
    keywordchoices.setValue(selectBoard[fbc.GKBOARD_KEYWORDS]);
    accesschoices.setChoiceByValue(selectBoard[fbc.GKBOARD_ACCESSUSERUIDARRAY]);

    $('#boardstatusswitch')
      .prop('checked', selectBoard[fbc.GKBOARD_STATUS])
      .trigger('change');
  }, [selectBoard]);
  ...
  const addorUpdateBoard = async () => {
    if (checkifDataisCorrect()) {
      ...
      console.log({ selectBoard });
      var boardUID =
        selectBoard !== null ? selectBoard[fbc.GKBOARD_UID] : ulid();

      var boardObj = {
        [fbc.GKBOARD_UID]: boardUID,
        [fbc.GKBOARD_NAME]: utility.getinputValue('boardname'),
        [fbc.GKBOARD_CODE]: utility.getinputValue('boardcode'),
        [fbc.GKBOARD_FIELD]: utility.getinputValue('boardfield'),
        [fbc.GKBOARD_ACCESSUSERUIDARRAY]: accesschoices.getValue(true),
        [fbc.GKBOARD_KEYWORDS]: keywordchoices.getValue(true),
        [fbc.GKBOARD_STATUS]: $('#boardstatusswitch').is(':checked'),
      };
        ...
        uploadBytes(uploadRef, imageFile)
          .then(async (snapshot) => {
            getDownloadURL(snapshot.ref).then(async (downloadURL) => {
              boardObj[fbc.GKBOARD_THUMBNAILURL] = downloadURL;
              uploadBoard();
            });
          })
          .catch((error) => {
      ...
      var uploadBoard = async () => {
        ...
        var addorUpdateBoard = await RequestaddorUpdateGkBoard(boardObj, log);
        utility.hideloading();
