...
import AddorUpdateContent from './add-update-content-Gk';
import AddorUpdateLesson from './add-update-lesson-Gk';
...
const SubjectList = ({ courseuid, subjectuid }) => {
  ...
  function addNewLessonClick(id) {
    subjectDocs.map((subject) => {
      if (subject[fbc.GKSUBJECT_UID] === id) {
        setaddNewLesson(() => {
          return {
            subject: {
              uid: id,
              name: subject[fbc.GKSUBJECT_NAME],
              code: subject[fbc.GKSUBJECT_CODE],
            },
            course: subject[fbc.GKSUBJECT_COURSE],
          };
        });
      }
    });
    ...
    subjectDocs.map((subject) => {
      var tabbtn = ` <li class="nav-item g-5">
          <button
            ...
            data-bs-toggle="tab"
            id="tabbtn_${subject[fbc.GKSUBJECT_UID]}"
            href="#tab_${subject[fbc.GKSUBJECT_UID]}"
          >
             ${subject[fbc.GKSUBJECT_NAME]}
          </button>
        </li>`;
      ...
      var tabcontent = ` <div
          ...
          id="tab_${subject[fbc.GKSUBJECT_UID]}"
          role="tabpanel"
        >  
        ...
        <div class="d-flex flex-row align-items-center">
         <img  src="${
           subject[fbc.GKSUBJECT_THUMBNAILURL]
         }" class="mb-2 bg-white rounded shadow-md   w-50px h-50px"></img>
        <div class="d-flex flex-column align-items-start  ps-4">

        <span class="fs-5 fw-bold">${subject[fbc.GKSUBJECT_NAME]}</span>
        <span class="fs-7">${utility.padwithzero(
          subject[fbc.GKSUBJECT_LESSONSUIDARRAY].length
        )} Lessons</span>
        </div>
        </div>
         <div class="d-flex justify-content-end w-25">
                      <button
                        id="addlessonbtn_${subject[fbc.GKSUBJECT_UID]}"
                        type="button"
         ...
         class="accordion accordion-icon-toggle h-100" id="accordion_${
           subject[fbc.GKSUBJECT_UID]
         }">      
        </div>
        </div>
        </div>
      ...
      $(`#addlessonbtn_${subject[fbc.GKSUBJECT_UID]}`).on('click', function () {
        addNewLessonClick(this.id.replaceAll('addlessonbtn_', ''));
      });
      var lessonsArray = [];

      Object.keys(subject[fbc.GKSUBJECT_LESSONS]).map((key, index) => {
        lessonsArray.push(subject[fbc.GKSUBJECT_LESSONS][key]);
      });
      ...
      lessonsArray.map((lesson) => {
        ...
        var item = ` <div class="mb-5">
          ...
          <div id="lessondiv_${key}" class="accordion-header border-bottom rounded bg-light-primary  align-items-center  p-3 d-flex collapsed" data-bs-toggle="collapse" data-bs-target="#${key}_lesson">
            ...
            <div class="d-flex flex-row align-items-center justify-content-between w-100">
                ...
                <div class="d-flex flex-row align-items-center">
                            ...
                            }" class=" bg-white rounded shadow-md   w-30px h-30px"></img>
                            <div class="d-flex flex-column align-items-start ps-4">
                              <h3 class="fs-4 fw-bolder text-capitalize  mb-0">${
                                lesson.name
                              }</h3>
                            </div>
          ...
          <div id="${key}_lesson" class="collapse bg-light px-4 py-2 mt-2 rounded" data-bs-parent="#accordion_${
          subject[fbc.GKSUBJECT_UID]
        }">
          <ul
          id="lessontabs_${key}"
        ...
        $('#accordion_' + subject[fbc.GKSUBJECT_UID]).append(item);
      ...
      if (subject[fbc.GKSUBJECT_UID] === subjectuid) {
  ...
  async function addNewContentClick(id) {
    ...
    setaddNewContent(() => {
      return {
        lesson: {
          uid: id,
          name: lessonData[fbc.GKLESSON_NAME],
          code: lessonData[fbc.GKLESSON_CODE],
        },
        subject: lessonData[fbc.GKLESSON_SUBJECT],
        course: lessonData[fbc.GKLESSON_COURSE],
      };
    });
  }
  ...
  async function getLesson(lessonuid, isforce = false) {
    ...
    if (lessonDocs[lessonuid] !== undefined && !isforce) {
      ...
      utility.showloading();
      const docRef = doc(db, fbc.GKLESSON_COLLECTION, lessonuid);
  ...
  async function getLessonContent(lessonuid) {
    ...
    if (Object.keys(lessonData[fbc.GKLESSON_CONTENT]).length === 0) {
    ...
    Object.keys(lessonData[fbc.GKLESSON_CONTENT]).map((key) => {
      contentArray.push(lessonData[fbc.GKLESSON_CONTENT][key]);
    });

    contentArray.sort(function (x, y) {
      let a = x[fbc.GKCONTENT_FILE].filename,
        b = y[fbc.GKCONTENT_FILE].filename;
      return a == b ? 0 : a > b ? 1 : -1;
    });
    contentArray.map((content) => {
      var item = ` <div id="contentdiv_${
        content[fbc.GKCONTENT_UID]
      }" class=" draggable_${lessonuid} d-flex flex-row bg-white border rounded py-2 px-5 justify-content-between mb-2">
      ...
      <span class="fs-3 text-primary border-secondary border-end pe-3 fw-bold my-auto">
       ...
       ${(() => {
         switch (content[fbc.GKCONTENT_FILE].mime) {
          ...
          <div class="d-flex flex-column ps-4">
            ...
            <span class="fs-6 fw-bold mb-1">${
              content[fbc.GKCONTENT_HEADING]
            }</span>
            <span class="text-xs text-muted text-wrap">${
              content[fbc.GKCONTENT_DESCRIPTION]
            }</span>
          ...
          <div class='d-flex flex-row align-items-center gap-2 py-2'>       
            <button
             data-filename="${content[fbc.GKCONTENT_FILE].filename}"
              id="viewcontent_${content[fbc.GKCONTENT_UID]}_${lessonuid}"  
            ...
            id="deletecontent_${
              content[fbc.GKCONTENT_UID]
            }_${lessonuid}" class="btn  h-30px text-xs px-2 py-1 btn-sm btn-light-danger">
      ...
      $(`#viewcontent_${content[fbc.GKCONTENT_UID]}_${lessonuid}`).on(
        'click',
        function (e) {
      ...
      $(`#deletecontent_${content[fbc.GKCONTENT_UID]}_${lessonuid}`).on(
        'click',
        function (e) {
        ...
        function (e) {
          window.open(
            window.location.origin +
              '/masters/edit-assessment?lesson=' +
              this.id.split('_')[2] +
              '&id=' +
              this.id.split('_')[1],
            '_blank'
  ...
  async function deleteContent(lessonuid, contentuid) {
    ...
    const contentRef = doc(db, fbc.GKCONTENT_COLLECTION, contentuid);
    const lessonRef = doc(db, fbc.GKLESSON_COLLECTION, lessonuid);
    batch.set(
      contentRef,
      {
        [fbc.GKLESSON_STATUS]: false,
        [fbc.GKCONTENT_LOGS]: arrayUnion({
    ...
    batch.set(
      lessonRef,
      {
        [fbc.GKLESSON_DELETEDCONTENT]: {
          [contentuid]: lessonData[fbc.GKLESSON_CONTENT][contentuid],
        },
        [fbc.GKLESSON_CONTENT]: {
  ...
  async function getAllSubjects(uid) {
    ...
    utility.showloading();
    try {
      const SubjectReF = collection(db, fbc.GKSUBJECT_COLLECTION);
      var allSubjects = [];
      var params = [where(`${fbc.GKSUBJECT_COURSE}.uid`, '==', uid)];
      if (!utility.get_keyvalue(USER_ISADMIN)) {
        params.push(
          where(
            fbc.GKSUBJECT_ACCESSUSERUIDARRAY,
            'array-contains',
            utility.get_keyvalue(USER_ID)
          )
        );
      }
      let q = query(SubjectReF, ...params, orderBy(fbc.GKSUBJECT_NAME));
      const querySnapshot = await getDocs(q);
  ...
  return (
    <>
      ...
      <div className="tab-content h-100" id="subjectTabContent"></div>

      <AddorUpdateLesson
        addNewLessonDetails={addNewLesson}
        selectedLesson={selectLesson}
        setLessonModal={setaddLessonModal}
      />

      <AddorUpdateContent
        addNewContentDetails={addNewContent}
        selectedContent={selectContent}
        setContentModal={setaddContentModal}
      />

      <AddorUpdateVideo
        addNewVideoDetails={addNewVideo}
