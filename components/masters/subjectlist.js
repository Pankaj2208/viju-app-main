...
const SubjectList = ({ courseuid, subjectuid }) => {
    ...
    subjectDocs.map((subject) => {
      ...
      var tabcontent = ` <div
        ...
        <div class="d-flex flex-row align-items-center">
         <img  src="${
           subject[fbc.SUBJECT_THUMBNAILURL]
        ...
        <span class="fs-7">${utility.padwithzero(
          subject[fbc.SUBJECT_LESSONSUIDARRAY].length
        )} Lessons</span>
        </div>
        </div>
  ...
  async function getLessonContent(lessonuid) {
    ...
    contentArray.map((content) => {
      ...
      $(`#viewcontent_${content[fbc.CONTENT_UID]}_${lessonuid}`).on(
        'click',
        function (e) {
      ...
      $(`#deletecontent_${content[fbc.CONTENT_UID]}_${lessonuid}`).on(
        'click',
        function (e) {
  ...
  async function getAllSubjects(uid) {
    ...
    try {
      ...
      var allSubjects = [];
      var params = [where(`${fbc.SUBJECT_COURSE}.uid`, '==', uid)];
      if (!utility.get_keyvalue(USER_ISADMIN)) {
        params.push(
          where(
            fbc.SUBJECT_ACCESSUSERUIDARRAY,
            'array-contains',
