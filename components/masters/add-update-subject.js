...
const AddorUpdateSubject = ({
  ...
  useEffect(() => {
    ...
    accesschoices.setChoiceByValue(
      selectSubject[fbc.SUBJECT_ACCESSUSERUIDARRAY]
    );
  ...
  async function getAllCourses() {
    ...
    try {
      ...
      let q = query(CourseReF, ...params, orderBy(fbc.COURSE_NAME));
  ...
  const addorUpdateSubject = async () => {
    if (checkifDataisCorrect()) {
      ...
      console.log({ selectSubject });
      var subjectUID =
        selectSubject !== null ? selectSubject[fbc.SUBJECT_UID] : ulid();
      ...
      var uploadSubject = async () => {
        ...
        var addorUpdateSubject = await RequestaddorUpdateSubject(
          subjectObj,
          log
        );
        utility.hideloading();
